...
public class DadaDeliveryService implements IDelivery {
    ...
    public Tips send(CoordinateSystem coordinateSystem, DeliverBaseOrder deliverBaseOrder) {
        ...
        String sendToDadaResult;
        DadaOrderAddResult dadaOrderAddResult;
        try {
            sendToDadaResult = dadaClient.deliver(orderParam, coordinateSystem.isNeedConvert());
            ...
            if (2105 == dadaOrderAddResult.getCode()) {
                sendToDadaResult = dadaClient.redeliver(orderParam, coordinateSystem.isNeedConvert());
                dadaOrderAddResult = Jackson.object(sendToDadaResult, DadaOrderAddResult.class);
    ...
    public Tips callback(String backMsg) {
        Map<String, String> resultMap = Maps.generic(Jackson.map(backMsg));
